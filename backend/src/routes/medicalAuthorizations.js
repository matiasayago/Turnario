...
function sessionId(req) {
  const u = req.user;
  if (!u) return '';
  return u._id != null ? u._id.toString() : String(u.id || '');
}
...
// POST solicitud (solo profesional)
router.post(
  '/',
  [
    body('patientId').trim().notEmpty().withMessage('patientId requerido'),
    body('authorizationType')
      .optional()
      .isIn(['full_access', 'limited_access', 'consultation_only'])
  ...
  async (req, res) => {
    try {
      ...
      const type = req.body.authorizationType || 'limited_access';
...
// PATCH rechazar solicitud (solo paciente, pending)
router.patch('/:id/reject', [param('id').trim().notEmpty()], async (req, res) => {
  try {
    if (req.user.userType !== 'client') {
...
// PATCH revocar acceso activo (paciente o profesional)
router.patch(
  '/:id/revoke',
...
// DELETE cancelar solicitud pendiente (solo profesional dueño)
